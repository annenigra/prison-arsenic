...
# Source water type
cws1$Source.Water.Type <- gsub(" ", "", cws1$Source.Water.Type) #Removes space 
table(cws1$Source.Water.Type)
cws1$Source.Water.Type[which(cws1$Source.Water.Type=="GWP")]<-"GW"
cws1$Source.Water.Type[which(cws1$Source.Water.Type=="GU")]<-"GW"
cws1$Source.Water.Type[which(cws1$Source.Water.Type=="SWP")]<-"SW"
table(cws1$Source.Water.Type)

cws3$Source.Water.Type <- gsub(" ", "", cws3$Source.Water.Type) #Removes space 
table(cws3$Source.Water.Type)
cws3$Source.Water.Type[which(cws3$Source.Water.Type=="GWP")]<-"GW"
cws3$Source.Water.Type[which(cws3$Source.Water.Type=="GU")]<-"GW"
cws3$Source.Water.Type[which(cws3$Source.Water.Type=="SWP")]<-"SW"
table(cws3$Source.Water.Type)
...
cws1$popcat<-0
cws1$popcat[cws1$Adjusted.Total.Population.Served<501]<-1
cws1$popcat[501<=cws1$Adjusted.Total.Population.Served&cws1$Adjusted.Total.Population.Served<3301]<-2
cws1$popcat[3301<=cws1$Adjusted.Total.Population.Served&cws1$Adjusted.Total.Population.Served<10001]<-3
cws1$popcat[10001<=cws1$Adjusted.Total.Population.Served&cws1$Adjusted.Total.Population.Served<100001]<-4
cws1$popcat[100001<=cws1$Adjusted.Total.Population.Served]<-5
table(cws1$popcat)

cws3$popcat<-0
cws3$popcat[cws3$Adjusted.Total.Population.Served<501]<-1
cws3$popcat[501<=cws3$Adjusted.Total.Population.Served&cws3$Adjusted.Total.Population.Served<3301]<-2
cws3$popcat[3301<=cws3$Adjusted.Total.Population.Served&cws3$Adjusted.Total.Population.Served<10001]<-3
cws3$popcat[10001<=cws3$Adjusted.Total.Population.Served&cws3$Adjusted.Total.Population.Served<100001]<-4
cws3$popcat[100001<=cws3$Adjusted.Total.Population.Served]<-5
table(cws3$popcat)
...
# Create long-form datasets for 3 and 1 year models
library(tidyr)
library(gee)  #for Generalized Estimating Equations  (function gee)            
cwslong<-gather(cws1, year, arsenic, YearlyAverage.2006:YearlyAverage.2011, factor_key=TRUE);dim(cwslong) 
# Save and read is necessary to avoid in GEE models the error "NAs introduced by coercion"  
write.csv(cwslong, file="~/Google Drive/Research/Prison Water As/cwslong.csv", row.names=F)
cwslong1 <- read.csv(file="~/Google Drive/Research/Prison Water As/cwslong.csv") ; dim(cwslong1) #1727 1415
# Order by PWSID is necessary to avoid in GEE model the error "Working correlation estimate not positive definite" 
cwslong1 <- cwslong1[order(cwslong1$PWSID),]
cwslong3<-gather(cws3, year, arsenic, ThreeYearAs20062008:ThreeYearAs20092011, factor_key=TRUE);dim(cwslong3) 
cwslong3 <- cwslong3[order(cwslong3$PWSID),]

# MCL exceedance
cwslong1$MCLExc<-0
cwslong1$MCLExc[which(cwslong1$arsenic>10)]<-1
table(cwslong1$MCLExc)
cwslong3$MCLExc<-0
...
## Extras for main EPA paper analysis:
# Generate dummy var for post time periods
cwslong1$post<-0
cwslong1$post[which(cwslong1$year!="YearlyAverage.2006")]<-1
table(cwslong1$post)
table(cwslong1$year)
# Generate dummy var for SW region (eg treatment) 
cwslong1$sw<-0; cwslong1$sw[which(cwslong1$Region=="SW")]<-1
# Generate interaction between post and treatment (SW)
cwslong1$post_sw<-(cwslong1$post)*(cwslong1$sw)





## Part 1. Independent correlation structure, only crude models:
### 1. Yearly avearges:
# First, let's model the effect of being in the SW on odds of exceedence over time:
# We get meaningfully different answers when using "exchangeable" , how about unstructured?  This is the most uninformative
fit <- gee(MCLExc~post+sw+post_sw,  id=as.factor(PWSID), data = cwslong1, corstr="unstructured" , family = binomial(link = "logit") )
summary(fit)
#               Estimate Naive S.E.    Naive z Robust S.E.   Robust z
#(Intercept) -4.40163162 0.05024234 -87.608012  0.05427204 -81.103118
#post        -0.05687872 0.04127353  -1.378092  0.05002479  -1.137011
#sw           1.07135313 0.07386578  14.504053  0.08222364  13.029745
#post_sw      0.35492630 0.05935078   5.980145  0.07033443   5.046267

# Interpretation:
# exp(-0.05687872) = 0.944; OR of MCL exceedence for first vs second time period in the non-SW
# exp(1.07135313) = 2.92; OR of MCL exceedence in SW compared to non-SW at baseline
# exp( 1.07135313+0.35492630)= 4.16; OR of MCL exceeence for SW vs non-southwest during the second time period
# exp(-0.05687872+0.35492630) = 1.347; OR of MCL exceedence at the second time period vs first time period, in the SW

beta <- summary(fit)$coeff[3, "Estimate"]
se <- summary( fit )$coeff[ 3 , "Robust S.E." ]
resultexamp <- convert.to.ci(round(exp(c(
  beta , 
  beta - 1.96 * se ,
  beta + 1.96 * se)), 2))
resultexamp
...
### 1. Yearly avearges:
# Specific models for prisons:
fit <- gee(MCLExc~IsPrison,  id=as.factor(PWSID), data = cwslong1, corstr="independence" , family = binomial(link = "logit") )
beta <- summary(fit)$coeff[2, "Estimate"]
se <- summary( fit )$coeff[ 2 , "Robust S.E." ]
resultall1 <- convert.to.ci(round(exp(c(
  beta , 
  beta - 1.96 * se ,
  beta + 1.96 * se)), 2))
resultall1
...
# Restrict to the SW:
swlong<-cwslong1[which(cwslong1$Region=="SW"),]
fit <- gee(MCLExc~IsPrison,  id=as.factor(PWSID), data = swlong, corstr="independence" , family = binomial(link = "logit") )
beta <- summary(fit)$coeff[2, "Estimate"]
se <- summary( fit )$coeff[ 2 , "Robust S.E." ]
resultsw1 <- convert.to.ci(round(exp(c(
  beta , 
  beta - 1.96 * se ,
  beta + 1.96 * se)), 2))
resultsw1
...
fit <- gee(MCLExc~IsPrison,  id=as.factor(PWSID), data = cwslong3, corstr="independence" , family = binomial(link = "logit") )
beta <- summary(fit)$coeff[2, "Estimate"]
se <- summary( fit )$coeff[ 2 , "Robust S.E." ]
resultall3 <- convert.to.ci(round(exp(c(
  beta , 
  beta - 1.96 * se ,
  beta + 1.96 * se)), 2))
resultall3
...
# Restrict to the SW:
swlong3<-cwslong3[which(cwslong3$Region=="SW"),]
fit <- gee(MCLExc~IsPrison,  id=as.factor(PWSID), data = swlong3, corstr="independence" , family = binomial(link = "logit") )
beta <- summary(fit)$coeff[2, "Estimate"]
se <- summary( fit )$coeff[ 2 , "Robust S.E." ]
resultsw3 <- convert.to.ci(round(exp(c(
  beta , 
  beta - 1.96 * se ,
  beta + 1.96 * se)), 2))
resultsw3
...
overall<-rbind(resultall1,resultall3,resultall6)
colnames(overall)[1]<-"Overall CWSs"
rownames(overall)[1]<-"One-year averages"
rownames(overall)[2]<-"Three-year averages"
rownames(overall)[3]<-"Six-year averages"


swresults<-rbind(resultsw1,resultsw3,resultsw6)
colnames(swresults)[1]<-"SW CWSs"
rownames(swresults)[1]<-"One-year averages"
rownames(swresults)[2]<-"Three-year averages"
rownames(swresults)[3]<-"Six-year averages"

ORResultsindependent<-cbind(overall,swresults)
write.xlsx(ORResultsindependent,row.names=T, file="~/Google Drive/Research/Prison Water As/output/Table 2 ORs.xlsx")


## Part 2. Unstructured correlation structure, adding adjusted models:
### 1. Yearly avearges:
fit <- gee(MCLExc~IsPrison,  id=as.factor(PWSID), data = cwslong1, corstr="unstructured" , family = binomial(link = "logit") )
beta <- summary(fit)$coeff[2, "Estimate"]
se <- summary( fit )$coeff[ 2 , "Robust S.E." ]
resultall1 <- convert.to.ci(round(exp(c(
  beta , 
...
resultall1
# Model 2
fit <- gee(MCLExc~IsPrison+ as.factor(Source.Water.Type) +as.factor(popcat),  id=as.factor(PWSID), data = cwslong1, corstr="unstructured" , family = binomial(link = "logit") )
beta <- summary(fit)$coeff[2, "Estimate"]
se <- summary( fit )$coeff[ 2 , "Robust S.E." ]
resultall1m2 <- convert.to.ci(round(exp(c(
  beta , 
  beta - 1.96 * se ,
  beta + 1.96 * se)), 2))
resultall1m2
...
# Restrict to the SW:
swlong<-cwslong1[which(cwslong1$Region=="SW"),]
fit <- gee(MCLExc~IsPrison,  id=as.factor(PWSID), data = swlong, corstr="unstructured" , family = binomial(link = "logit") )
beta <- summary(fit)$coeff[2, "Estimate"]
se <- summary( fit )$coeff[ 2 , "Robust S.E." ]
resultsw1 <- convert.to.ci(round(exp(c(
  beta , 
...
# Model 2
swlong<-cwslong1[which(cwslong1$Region=="SW"),]
fit <- gee(MCLExc~IsPrison+ as.factor(Source.Water.Type) +as.factor(popcat),  id=as.factor(PWSID), data = swlong, corstr="unstructured" , family = binomial(link = "logit") )
beta <- summary(fit)$coeff[2, "Estimate"]
se <- summary( fit )$coeff[ 2 , "Robust S.E." ]
resultsw1m2 <- convert.to.ci(round(exp(c(
  beta , 
  beta - 1.96 * se ,
  beta + 1.96 * se)), 2))
resultsw1m2
...
### 2. Three-year averages:
fit <- gee(MCLExc~IsPrison,  id=as.factor(PWSID), data = cwslong3, corstr="unstructured" , family = binomial(link = "logit") )
beta <- summary(fit)$coeff[2, "Estimate"]
se <- summary( fit )$coeff[ 2 , "Robust S.E." ]
resultall3 <- convert.to.ci(round(exp(c(
  beta , 
...
resultall3
# Model 2:
fit <- gee(MCLExc~IsPrison+ as.factor(Source.Water.Type) +as.factor(popcat),  id=as.factor(PWSID), data = cwslong3, corstr="unstructured" , family = binomial(link = "logit") )
beta <- summary(fit)$coeff[2, "Estimate"]
se <- summary( fit )$coeff[ 2 , "Robust S.E." ]
resultall3m2 <- convert.to.ci(round(exp(c(
  beta , 
  beta - 1.96 * se ,
  beta + 1.96 * se)), 2))
resultall3m2
...
# Restrict to the SW:
swlong3<-cwslong3[which(cwslong3$Region=="SW"),]
fit <- gee(MCLExc~IsPrison,  id=as.factor(PWSID), data = swlong3, corstr="unstructured" , family = binomial(link = "logit") )
beta <- summary(fit)$coeff[2, "Estimate"]
se <- summary( fit )$coeff[ 2 , "Robust S.E." ]
resultsw3 <- convert.to.ci(round(exp(c(
  beta , 
...
resultsw3
# Model 2
fit <- gee(MCLExc~IsPrison+ as.factor(Source.Water.Type) +as.factor(popcat),  id=as.factor(PWSID), data = swlong3, corstr="unstructured" , family = binomial(link = "logit") )
beta <- summary(fit)$coeff[2, "Estimate"]
se <- summary( fit )$coeff[ 2 , "Robust S.E." ]
resultsw3m2 <- convert.to.ci(round(exp(c(
  beta , 
  beta - 1.96 * se ,
  beta + 1.96 * se)), 2))
resultsw3m2
...
beta <- summary(fit)$coeff[2, "Estimate"]
se <- summary( fit )$coeff[ 2 , "Std. Error" ]
resultall6m2 <- convert.to.ci(round(exp(c(
  beta , 
  beta - 1.96 * se ,
  beta + 1.96 * se)), 2))
resultall6m2
...
beta <- summary(fit)$coeff[2, "Estimate"]
se <- summary( fit )$coeff[ 2 , "Std. Error" ]
resultsw6m2 <- convert.to.ci(round(exp(c(
  beta , 
  beta - 1.96 * se ,
  beta + 1.96 * se)), 2))
resultsw6m2

overall<-rbind(resultall1,resultall1m2,resultall3,resultall3m2,resultall6,resultall6m2)
colnames(overall)[1]<-"Overall CWSs"
rownames(overall)[1]<-"One-year averages, model 1"
rownames(overall)[2]<-"One-year averages, model 2"
rownames(overall)[3]<-"Three-year averages, model 1"
rownames(overall)[4]<-"Three-year averages, model 2"
rownames(overall)[5]<-"Six-year averages, model 1"
rownames(overall)[6]<-"Six-year averages, model 2"

swresults<-rbind(resultsw1,resultsw1m2,resultsw3,resultsw3m2,resultsw6,resultsw6m2)
colnames(swresults)[1]<-"SW CWSs"
rownames(swresults)[1]<-"One-year averages, model 1"
rownames(swresults)[2]<-"One-year averages, model 2"
rownames(swresults)[3]<-"Three-year averages, model 1"
rownames(swresults)[4]<-"Three-year averages, model 2"
rownames(swresults)[5]<-"Six-year averages, model 1"
rownames(swresults)[6]<-"Six-year averages, model 2"

ORResultsunstructured<-cbind(overall,swresults)
write.xlsx(ORResultsunstructured,row.names=T, file="~/Google Drive/Research/Prison Water As/output/Table 2 ORs unstructured.xlsx")
...
####### 7. Extract all yearly and 6-year averages for prisons #######

prisvalues<-merge(cws6,wideyearlycorrected,by="PWSID")
myvars<-c("PWSID","System.Name","State.Code","Adjusted.Total.Population.Served","Source.Water.Type",
          "SixYearAs", "YearlyAverage.2006","YearlyAverage.2007","YearlyAverage.2008","YearlyAverage.2009",
          "YearlyAverage.2010","YearlyAverage.2011","IsPrison")
prisvalues<-prisvalues[myvars]
prisonrecords<-prisvalues[which(prisvalues$IsPrison==1),]
prisonrecords<-prisonrecords[order(-prisonrecords$SixYearAs),]

write.xlsx(prisonrecords,row.names=T, file="~/Google Drive/Research/Prison Water As/output/Prison System Records.xlsx")
...
####### 8. Not shown: Supp Table 2. Distribution of 3-year avearges by prison status and region ######
myvars<-c("PWSID","ThreeYearAs20062008","ThreeYearAs20092011")
tomerge<-pcwsy3[myvars]
cws6A<-merge(cws6,tomerge,by="PWSID",all.x=T)
cws6<-cws6A

## Non-Prison CWSs in the non-Southwest (N=28,204)
# Time 1
data1<-cws6[which(cws6$Region!="SW"&cws6$IsPrison==0),];nrow(data1)
nav<-nrow(data1[which(!is.na(data1$ThreeYearAs20062008)),])
nmiss<-nrow(data1[which(is.na(data1$ThreeYearAs20062008)),])
pop<-sum(data1$Adjusted.Total.Population.Served)
quant<-as.data.frame(quantile(data1$ThreeYearAs20062008,  probs = c(0,0.1, 0.25, 0.5, 0.75, 0.9,0.95, 0.99,1), na.rm=TRUE, scientific=F))
nmrl1<-nrow(data1[which(data1$ThreeYearAs20062008<1),])
nmrl<-round(100*(nmrl1/nav),1)
nmcl1<-nrow(data1[which(data1$ThreeYearAs20062008>10),])
nmcl<-round(100*(nmcl1/nav),1)
nmclfin<-to.n.prop(c(nmcl1,nmcl))
gm<-exp(mean(log(data1$ThreeYearAs20062008),na.rm=T));gm<-round(gm,digits=2)
mean<-mean(data1$ThreeYearAs20062008,na.rm=T);mean<-round(mean,digits=2)
colnames(quant)[1] <- "Other CWSs"
Time1<-rbind(nav,nmiss,pop,quant,nmrl,nmclfin,gm,mean)
rownames(Time1)[1] <- "N";rownames(Time1)[2] <- "N missing";rownames(Time1)[13] <- "% below MRL"
rownames(Time1)[14] <- "N (%) above MCL";rownames(Time1)[15] <- "Geometric mean";rownames(Time1)[16] <- "Arithmetic mean"
nonSWCWSs<-Time1
nonSWCWSs1 <- rbind(nonSWCWSs, Region= c("non-Southwest"))

# Time 2
data1<-cws6[which(cws6$Region!="SW"&cws6$IsPrison==0),];nrow(data1)
nav<-nrow(data1[which(!is.na(data1$ThreeYearAs20092011)),])
nmiss<-nrow(data1[which(is.na(data1$ThreeYearAs20092011)),])
pop<-sum(data1$Adjusted.Total.Population.Served)
quant<-as.data.frame(quantile(data1$ThreeYearAs20092011,  probs = c(0,0.1, 0.25, 0.5, 0.75, 0.9,0.95, 0.99,1), na.rm=TRUE, scientific=F))
nmrl1<-nrow(data1[which(data1$ThreeYearAs20092011<1),])
nmrl<-round(100*(nmrl1/nav),1)
nmcl1<-nrow(data1[which(data1$ThreeYearAs20092011>10),])
nmcl<-round(100*(nmcl1/nav),1)
nmclfin<-to.n.prop(c(nmcl1,nmcl))
gm<-exp(mean(log(data1$ThreeYearAs20092011),na.rm=T));gm<-round(gm,digits=2)
mean<-mean(data1$ThreeYearAs20092011,na.rm=T);mean<-round(mean,digits=2)
colnames(quant)[1] <- "Other CWSs"
Time2<-rbind(nav,nmiss,pop,quant,nmrl,nmclfin,gm,mean)
rownames(Time2)[1] <- "N";rownames(Time1)[2] <- "N missing";rownames(Time1)[13] <- "% below MRL"
rownames(Time2)[14] <- "N (%) above MCL";rownames(Time1)[15] <- "Geometric mean";rownames(Time1)[16] <- "Arithmetic mean"
nonSWCWSs<-Time2
nonSWCWSs2 <- rbind(nonSWCWSs, Region= c("non-Southwest"))


## Non-Prison CWSs in the SW (N=8,727)
# Time 1
data2<-cws6[which(cws6$Region=="SW"&cws6$IsPrison==0),];nrow(data2)
nav<-nrow(data2[which(!is.na(data2$ThreeYearAs20062008)),])
nmiss<-nrow(data2[which(is.na(data2$ThreeYearAs20062008)),])
pop<-sum(data2$Adjusted.Total.Population.Served)
quant<-as.data.frame(quantile(data2$ThreeYearAs20062008,  probs = c(0,0.1, 0.25, 0.5, 0.75, 0.9,0.95, 0.99,1), na.rm=TRUE, scientific=F))
nmrl1<-nrow(data2[which(data2$ThreeYearAs20062008<1),])
nmrl<-round(100*(nmrl1/nav),1)
nmcl1<-nrow(data2[which(data2$ThreeYearAs20062008>10),])
nmcl<-round(100*(nmcl1/nav),1)
nmclfin<-to.n.prop(c(nmcl1,nmcl))
gm<-exp(mean(log(data2$ThreeYearAs20062008),na.rm=T));gm<-round(gm,2)
mean<-mean(data2$ThreeYearAs20062008,na.rm=T);mean<-round(mean,2)
colnames(quant)[1] <- "Other CWSs"
Time1<-rbind(nav,nmiss,pop,quant,nmrl,nmclfin,gm,mean)
rownames(Time1)[1] <- "N";rownames(Time1)[2] <- "N missing";rownames(Time1)[13] <- "% below MRL"
rownames(Time1)[14] <- " N (%) above MCL";rownames(Time1)[15] <- "Geometric mean";rownames(Time1)[16] <- "Arithmetic mean"
SWCWSs<-Time1
SWCWSs1 <- rbind(SWCWSs, Region= c("Southwest"))


# Time 2
data2<-cws6[which(cws6$Region=="SW"&cws6$IsPrison==0),];nrow(data2)
nav<-nrow(data2[which(!is.na(data2$ThreeYearAs20092011)),])
nmiss<-nrow(data2[which(is.na(data2$ThreeYearAs20092011)),])
pop<-sum(data2$Adjusted.Total.Population.Served)
quant<-as.data.frame(quantile(data2$ThreeYearAs20092011,  probs = c(0,0.1, 0.25, 0.5, 0.75, 0.9,0.95, 0.99,1), na.rm=TRUE, scientific=F))
nmrl1<-nrow(data2[which(data2$ThreeYearAs20092011<1),])
nmrl<-round(100*(nmrl1/nav),1)
nmcl1<-nrow(data2[which(data2$ThreeYearAs20092011>10),])
nmcl<-round(100*(nmcl1/nav),1)
nmclfin<-to.n.prop(c(nmcl1,nmcl))
gm<-exp(mean(log(data2$ThreeYearAs20092011),na.rm=T));gm<-round(gm,2)
mean<-mean(data2$ThreeYearAs20092011,na.rm=T);mean<-round(mean,2)
colnames(quant)[1] <- "Other CWSs"
Time2<-rbind(nav,nmiss,pop,quant,nmrl,nmclfin,gm,mean)
rownames(Time2)[1] <- "N";rownames(Time1)[2] <- "N missing";rownames(Time1)[13] <- "% below MRL"
rownames(Time2)[14] <- " N (%) above MCL";rownames(Time1)[15] <- "Geometric mean";rownames(Time1)[16] <- "Arithmetic mean"
SWCWSs<-Time2
SWCWSs2 <- rbind(SWCWSs, Region= c("Southwest"))


## Prisons in the non-SW (N=132)
# Time 1
data3<-cws6[which(cws6$Region!="SW"&cws6$IsPrison==1),];nrow(data3)
nav<-nrow(data3[which(!is.na(data3$ThreeYearAs20062008)),])
nmiss<-nrow(data3[which(is.na(data3$ThreeYearAs20062008)),])
pop<-sum(data3$Adjusted.Total.Population.Served)
quant<-as.data.frame(quantile(data3$ThreeYearAs20062008,  probs = c(0,0.1, 0.25, 0.5, 0.75, 0.9,0.95, 0.99,1), na.rm=TRUE, scientific=F))
nmrl1<-nrow(data3[which(data3$ThreeYearAs20062008<1),])
nmrl<-round(100*(nmrl1/nav),1)
nmcl1<-nrow(data3[which(data3$ThreeYearAs20062008>10),])
nmcl<-round(100*(nmcl1/nav),1)
nmclfin<-to.n.prop(c(nmcl1,nmcl))
gm<-exp(mean(log(data3$ThreeYearAs20062008),na.rm=T));gm<-round(gm,2)
mean<-mean(data3$ThreeYearAs20062008,na.rm=T);mean<-round(mean,2)
colnames(quant)[1] <- "Correctional facilities"
Time1<-rbind(nav,nmiss,pop,quant,nmrl,nmclfin,gm,mean)
rownames(Time1)[1] <- "N";rownames(Time1)[2] <- "N missing";rownames(Time1)[13] <- "% below MRL"
rownames(Time1)[14] <- "N (%) above MCL";rownames(Time1)[15] <- "Geometric mean";rownames(Time1)[16] <- "Arithmetic mean"
nonSWPrisons<-Time1
nonSWPrisons1 <- rbind(nonSWPrisons, Region= c("non-Southwest"))

# Time 2
data3<-cws6[which(cws6$Region!="SW"&cws6$IsPrison==1),];nrow(data3)
nav<-nrow(data3[which(!is.na(data3$ThreeYearAs20092011)),])
nmiss<-nrow(data3[which(is.na(data3$ThreeYearAs20092011)),])
pop<-sum(data3$Adjusted.Total.Population.Served)
quant<-as.data.frame(quantile(data3$ThreeYearAs20092011,  probs = c(0,0.1, 0.25, 0.5, 0.75, 0.9,0.95, 0.99,1), na.rm=TRUE, scientific=F))
nmrl1<-nrow(data3[which(data3$ThreeYearAs20092011<1),])
nmrl<-round(100*(nmrl1/nav),1)
nmcl1<-nrow(data3[which(data3$ThreeYearAs20092011>10),])
nmcl<-round(100*(nmcl1/nav),1)
nmclfin<-to.n.prop(c(nmcl1,nmcl))
gm<-exp(mean(log(data3$ThreeYearAs20092011),na.rm=T));gm<-round(gm,2)
mean<-mean(data3$ThreeYearAs20092011,na.rm=T);mean<-round(mean,2)
colnames(quant)[1] <- "Correctional facilities"
Time2<-rbind(nav,nmiss,pop,quant,nmrl,nmclfin,gm,mean)
rownames(Time2)[1] <- "N";rownames(Time1)[2] <- "N missing";rownames(Time1)[13] <- "% below MRL"
rownames(Time2)[14] <- "N (%) above MCL";rownames(Time1)[15] <- "Geometric mean";rownames(Time1)[16] <- "Arithmetic mean"
nonSWPrisons<-Time2
nonSWPrisons2 <- rbind(nonSWPrisons, Region= c("non-Southwest"))


## Prisons in the SW (N=23)
# Time 1
data4<-cws6[which(cws6$Region=="SW"&cws6$IsPrison==1),];nrow(data4)
nav<-nrow(data4[which(!is.na(data4$ThreeYearAs20062008)),])
nmiss<-nrow(data4[which(is.na(data4$ThreeYearAs20062008)),])
pop<-sum(data4$Adjusted.Total.Population.Served)
quant<-as.data.frame(quantile(data4$ThreeYearAs20062008,  probs = c(0,0.1, 0.25, 0.5, 0.75, 0.9,0.95, 0.99,1), na.rm=TRUE, scientific=F))
nmrl1<-nrow(data4[which(data4$ThreeYearAs20062008<1),])
nmrl<-round(100*(nmrl1/nav),1)
nmcl1<-nrow(data4[which(data4$ThreeYearAs20062008>10),])
nmcl<-round(100*(nmcl1/nav),1)
nmclfin<-to.n.prop(c(nmcl1,nmcl))
gm<-exp(mean(log(data4$ThreeYearAs20062008),na.rm=T));gm<-round(gm,2)
mean<-mean(data4$ThreeYearAs20062008,na.rm=T);mean<-round(mean,2)
colnames(quant)[1] <- "Correctional facilites"
Time1<-rbind(nav,nmiss,pop,quant,nmrl,nmclfin,gm,mean)
rownames(Time1)[1] <- "N";rownames(Time1)[2] <- "N missing";rownames(Time1)[13] <- "% below MRL"
rownames(Time1)[14] <- "N (%) above MCL";rownames(Time1)[15] <- "Geometric mean";rownames(Time1)[16] <- "Arithmetic mean"
SWPrisons<-Time1
SWPrisons1 <- rbind(SWPrisons, Region= c("Southwest"))
table(data4$State.Code)

# Time 2
data4<-cws6[which(cws6$Region=="SW"&cws6$IsPrison==1),];nrow(data4)
nav<-nrow(data4[which(!is.na(data4$ThreeYearAs20092011)),])
nmiss<-nrow(data4[which(is.na(data4$ThreeYearAs20092011)),])
pop<-sum(data4$Adjusted.Total.Population.Served)
quant<-as.data.frame(quantile(data4$ThreeYearAs20092011,  probs = c(0,0.1, 0.25, 0.5, 0.75, 0.9,0.95, 0.99,1), na.rm=TRUE, scientific=F))
nmrl1<-nrow(data4[which(data4$ThreeYearAs20092011<1),])
nmrl<-round(100*(nmrl1/nav),1)
nmcl1<-nrow(data4[which(data4$ThreeYearAs20092011>10),])
nmcl<-round(100*(nmcl1/nav),1)
nmclfin<-to.n.prop(c(nmcl1,nmcl))
gm<-exp(mean(log(data4$ThreeYearAs20092011),na.rm=T));gm<-round(gm,2)
mean<-mean(data4$ThreeYearAs20092011,na.rm=T);mean<-round(mean,2)
colnames(quant)[1] <- "Correctional facilites"
Time2<-rbind(nav,nmiss,pop,quant,nmrl,nmclfin,gm,mean)
rownames(Time2)[1] <- "N";rownames(Time1)[2] <- "N missing";rownames(Time1)[13] <- "% below MRL"
rownames(Time2)[14] <- "N (%) above MCL";rownames(Time1)[15] <- "Geometric mean";rownames(Time1)[16] <- "Arithmetic mean"
SWPrisons<-Time2
SWPrisons2 <- rbind(SWPrisons, Region= c("Southwest"))
table(data4$State.Code)

SuppTable2<-cbind(nonSWCWSs1,nonSWCWSs2,nonSWPrisons1,nonSWPrisons2,SWCWSs1,SWCWSs2,SWPrisons1,SWPrisons2)
write.xlsx(SuppTable2,row.names=T, file="~/Google Drive/Research/Prison Water As/output/Supp Table 2.xlsx")
...
SW$notprison[which(SW$IsPrison==1)]<-0
table(SW$notprison)
fit1 <- glm(ThreeYearAs20062008 ~-1+ notprison, family=gaussian(link="identity"), data=SW)
summary(fit1)
NoPrisonSW<-convert.to.ci(round(c(summary(fit1)$coeff[1,1],
                                  summary(fit1)$coeff[1,1]-qt(0.975, df=summary(fit1)$df[2])*summary(fit1)$coeff[1,2],
                                  summary(fit1)$coeff[1,1]+qt(0.975,df=summary(fit1)$df[2])*summary(fit1)$coeff[1,2]),2))
NoPrisonSW # "3.64 (3.45,3.82)"

## 2. Prisons in the SW:
fit1 <- glm(ThreeYearAs20062008 ~-1+ IsPrison, family=gaussian(link="identity"), data=SW)
summary(fit1)
PrisonsSW<-convert.to.ci(round(c(summary(fit1)$coeff[1,1],
                                 summary(fit1)$coeff[1,1]-qt(0.975, df=summary(fit1)$df[2])*summary(fit1)$coeff[1,2],
                                 summary(fit1)$coeff[1,1]+qt(0.975,df=summary(fit1)$df[2])*summary(fit1)$coeff[1,2]),2))
PrisonsSW #"3.64 (3.45,3.82)"
...
nonSW<-cws6[which(cws6$Region!="SW"),]
table(nonSW$notprison)
fit1 <- glm(ThreeYearAs20062008 ~-1+ notprison, family=gaussian(link="identity"), data=nonSW)
summary(fit1)
NoPrisonNonSW<-convert.to.ci(round(c(summary(fit1)$coeff[1,1],
                                     summary(fit1)$coeff[1,1]-qt(0.975, df=summary(fit1)$df[2])*summary(fit1)$coeff[1,2],
                                     summary(fit1)$coeff[1,1]+qt(0.975,df=summary(fit1)$df[2])*summary(fit1)$coeff[1,2]),2))
NoPrisonNonSW # "1.46 (1.4,1.51)"

## 2. Prisons in the non-SW:
fit1 <- glm(ThreeYearAs20062008 ~-1+ IsPrison, family=gaussian(link="identity"), data=nonSW)
summary(fit1)
PrisonNonSW<-convert.to.ci(round(c(summary(fit1)$coeff[1,1],
                                   summary(fit1)$coeff[1,1]-qt(0.975, df=summary(fit1)$df[2])*summary(fit1)$coeff[1,2],
                                   summary(fit1)$coeff[1,1]+qt(0.975,df=summary(fit1)$df[2])*summary(fit1)$coeff[1,2]),2))
PrisonNonSW #[1] "1.15 (0.31,1.99)"


## Prisons in CA specifically
CA<-cws6[which(cws6$State.Code=="CA"),];dim(CA)
fit1 <- glm(ThreeYearAs20062008 ~-1+ IsPrison, family=gaussian(link="identity"), data=CA)
summary(fit1)
CAprisons<-convert.to.ci(round(c(summary(fit1)$coeff[1,1],
                                 summary(fit1)$coeff[1,1]-qt(0.975, df=summary(fit1)$df[2])*summary(fit1)$coeff[1,2],
                                 summary(fit1)$coeff[1,1]+qt(0.975,df=summary(fit1)$df[2])*summary(fit1)$coeff[1,2]),2))
CAprisons #"8.17 (3.51,12.83)"
...
SW$notprison[which(SW$IsPrison==1)]<-0
table(SW$notprison)
fit1 <- glm(ThreeYearAs20092011 ~-1+ notprison, family=gaussian(link="identity"), data=SW)
summary(fit1)
NoPrisonSW<-convert.to.ci(round(c(summary(fit1)$coeff[1,1],
                                  summary(fit1)$coeff[1,1]-qt(0.975, df=summary(fit1)$df[2])*summary(fit1)$coeff[1,2],
                                  summary(fit1)$coeff[1,1]+qt(0.975,df=summary(fit1)$df[2])*summary(fit1)$coeff[1,2]),2))
NoPrisonSW # "3.22 (3.07,3.37)"

## 2. Prisons in the SW:
fit1 <- glm(ThreeYearAs20092011 ~-1+ IsPrison, family=gaussian(link="identity"), data=SW)
summary(fit1)
PrisonsSW<-convert.to.ci(round(c(summary(fit1)$coeff[1,1],
                                 summary(fit1)$coeff[1,1]-qt(0.975, df=summary(fit1)$df[2])*summary(fit1)$coeff[1,2],
                                 summary(fit1)$coeff[1,1]+qt(0.975,df=summary(fit1)$df[2])*summary(fit1)$coeff[1,2]),2))
PrisonsSW #"4.99 (1.83,8.14)"
...
nonSW<-cws6[which(cws6$Region!="SW"),]
table(nonSW$notprison)
fit1 <- glm(ThreeYearAs20092011 ~-1+ notprison, family=gaussian(link="identity"), data=nonSW)
summary(fit1)
NoPrisonNonSW<-convert.to.ci(round(c(summary(fit1)$coeff[1,1],
                                     summary(fit1)$coeff[1,1]-qt(0.975, df=summary(fit1)$df[2])*summary(fit1)$coeff[1,2],
                                     summary(fit1)$coeff[1,1]+qt(0.975,df=summary(fit1)$df[2])*summary(fit1)$coeff[1,2]),2))
NoPrisonNonSW # "1.29 (1.26,1.32)"

## 2. Prisons in the non-SW:
fit1 <- glm(ThreeYearAs20092011 ~-1+ IsPrison, family=gaussian(link="identity"), data=nonSW)
summary(fit1)
PrisonNonSW<-convert.to.ci(round(c(summary(fit1)$coeff[1,1],
                                   summary(fit1)$coeff[1,1]-qt(0.975, df=summary(fit1)$df[2])*summary(fit1)$coeff[1,2],
                                   summary(fit1)$coeff[1,1]+qt(0.975,df=summary(fit1)$df[2])*summary(fit1)$coeff[1,2]),2))
PrisonNonSW #[1]"1.15 (0.62,1.67)"


## Prisons in CA specifically
CA<-cws6[which(cws6$State.Code=="CA"),];dim(CA)
fit1 <- glm(ThreeYearAs20092011 ~-1+ IsPrison, family=gaussian(link="identity"), data=CA)
summary(fit1)
CAprisons<-convert.to.ci(round(c(summary(fit1)$coeff[1,1],
                                 summary(fit1)$coeff[1,1]-qt(0.975, df=summary(fit1)$df[2])*summary(fit1)$coeff[1,2],
                                 summary(fit1)$coeff[1,1]+qt(0.975,df=summary(fit1)$df[2])*summary(fit1)$coeff[1,2]),2))
CAprisons #"5.91 (1.49,10.34)"
