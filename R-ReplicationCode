...
describe(checking$PWSID) # 1,379 distinct PWSIDs reporting both raw and finished samples 
...
### end
#######     5.A. Mean (95% CI) arsenic concentrations by status (CWS vs. Prison) ######
...
p1 <- ggplot(data = cws6[which(cws6$F3Group=="NoPrisonsNoSW"|cws6$F3Group=="PrisonsNoSW"),], aes(y = log(SixYearAs), x = as.factor(F3Group), fill = F3Group)) +
  ...
  annotate(geom="text", x=1.8, y=1.13, label=expression(bold(paste("Mean (95% CI): 1.16 (0.58, 1.74) ", mu,"g/L"))),color="#D95F02",size=5)+ #prisons not in the SW "1.12 (0.34,1.9)"
  annotate(geom="text", x=0.8, y=1.13, label=expression(bold(paste("Mean (95% CI): 1.39 (1.35, 1.42) ", mu,"g/L"))),color="#1B9E77",size=5)+ #non-SW CWSs 1.38 (1.33,1.44)
...
p2 <- ggplot(data = cws6[which(cws6$F3Group=="SWNoPrisons"|cws6$F3Group=="SWPrisons"),], aes(y = log(SixYearAs), x = as.factor(F3Group), fill = F3Group)) +
  ...
  annotate(geom="text", x=1.8, y=1.13, label=expression(bold(paste("Mean (95% CI): 6.41 (3.48, 9.34) ", mu,"g/L"))),color="#D95F02",size=5)+ # Prisons in SW "6.49 (3.56,9.42)"
  annotate(geom="text", x=0.8, y=1.13, label=expression(bold(paste("Mean (95% CI): 3.11 (2.97, 3.24) ", mu,"g/L"))),color="#1B9E77",size=5)+ #Other CWSs in the SW  "3.11 (2.97,3.24)"
  annotate(geom="text", x=0.5, y=3.2, label=expression(bold(paste("Maximum contaminant level"))),color="tomato4",size=5)
